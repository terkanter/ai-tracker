import { Confirm } from "@workspace/admin/confirm";
import type { ArrayInputContextValue } from "@workspace/admin/hooks/array-input-context";
import { useArrayInput } from "@workspace/admin/hooks/array-input-context";
import type { SimpleFormIteratorItemContextValue } from "@workspace/admin/hooks/simple-form-iterator-context";
import {
  SimpleFormIteratorContext,
  SimpleFormIteratorItemContext,
  useSimpleFormIterator,
  useSimpleFormIteratorItem,
} from "@workspace/admin/hooks/simple-form-iterator-context";
import { IconButtonWithTooltip } from "@workspace/admin/icon-button-with-tooltip";
    ...
    return (
      <SimpleFormIteratorItemContext.Provider value={context}>
        <li
          ref={ref}
          className={cn(
            "border-border flex flex-row items-start justify-between gap-2 border-b pb-2 last:border-b-0",
        ...
        >
          ...
          <SourceContextProvider value={sourceContext}>
            <RecordContextProvider value={record}>
              <div
                className={cn(
                  "flex flex-1 gap-2",
                  inline ? "flex-col gap-2 sm:flex-row" : "flex-col",
                )}
              >
                {children}
              </div>
          ...
          {!disabled && (
            <div className="simple-form-iterator-item-actions flex h-9 flex-row items-center gap-1">
...
export function AddItemButton(props: React.ComponentProps<"button">) {
...
export function ReOrderButtons({ className }: { className?: string }) {
...
export function ClearArrayButton(props: React.ComponentProps<"button">) {
  const translate = useTranslate();
...
export function RemoveItemButton(props: React.ComponentProps<"button">) {
